...
BOOST_AUTO_TEST_SUITE( A3_test )

BOOST_AUTO_TEST_CASE( Invalid_Cmp_Constructor )
{
  std::shared_ptr<lyachko::Shape> nilshape( nullptr );
  BOOST_CHECK_THROW( lyachko::CompositeShape compositeshape( nilshape ), std::invalid_argument );
}

BOOST_AUTO_TEST_CASE( CMP_MoveTo )
{
  lyachko::point_t cmp_original_coord = { 1, 1 };
  lyachko::point_t new_coord = { 4, 4 };
 
  std::shared_ptr<lyachko::Shape> rectangle( new lyachko::Rectangle( { { cmp_original_coord }, 3,  3 } ) );
...
BOOST_AUTO_TEST_CASE( CMP_ShiftBy )
{
  lyachko::point_t cmp_original_coord = { 1, 1 };
  double dx = 3;
  double dy = 3;
...
BOOST_AUTO_TEST_CASE(  CMP_Scale  )
{
...
BOOST_AUTO_TEST_CASE( CMP_Add ) //Note that this test does not work correctly with areas crossing each other forming one single area
{
  std::shared_ptr<lyachko::Shape> rectangle( new lyachko::Rectangle( { { 0, 0 }, 2,  2 } ) );
  std::shared_ptr<lyachko::Shape> circle1( new lyachko::Circle( { 4, 4 }, 1 ) );
  std::shared_ptr<lyachko::Shape> circle2( new lyachko::Circle( { -1, -1 }, 0 ) ); //two of these points form a rectangle 10 x 10
  std::shared_ptr<lyachko::Shape> circle3( new lyachko::Circle( { 9, 9 }, 0 ) );
...
{
  std::shared_ptr<lyachko::Shape> circle1( new lyachko::Circle( { 0, 0 }, 0 ) );//Two points form a square 10 x 10, and there is a crciel inside with its own area;
  std::shared_ptr<lyachko::Shape> circle2( new lyachko::Circle( { 3, 3 }, 1 ) );;//Smallers squares should be 4 x 4 and 7 x 7;
  std::shared_ptr<lyachko::Shape> circle3( new lyachko::Circle( { 9, 9 }, 0 ) );;//Bigger one 9 x 9. Lets check that.
...
{
  std::shared_ptr<lyachko::Shape> circle( new lyachko::Circle( { 0, 0 }, 1 ) );
